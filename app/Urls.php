...
class Urls extends Model
{
    ...
    public static function DetectDeviceAndCount($id)
        {
          ...
          {
            DB::Table('urls')->whereid($id)->Increment('mobile_clicks');
            return 'mobile';
          } else {
            DB::Table('urls')->whereid($id)->Increment('desktop_clicks');
            return 'desktop';
          }
        }

    public function IncrementMobileClick()
    {
      DB::table('users')->whereId(Auth::user()->id)->increment('position');

    }

    public function IncrementDesktopClick($id)
    {
